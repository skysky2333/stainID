# StainID pipeline to segment the stains and analyze the results
python -m scripts.make_previews --input_dir data/TMA_core_exports --out_root results_stainID --max_per_type 6 --longest_side 2048

streamlit run scripts/calibrate_stain_streamlit.py

python -m scripts.segment_stain --input_dir data/TMA_core_exports --out_root results_stainID --types 212 213 214 215 --workers 4

python -m scripts.analyze_results --out_root results_stainID --types 212 213 214 215 --metadata data/brain_summary.csv
